...
app.post('/register', async (req, res) => {
    ...
    const re = dbHandler.checkAlreadyUser(username);
    let err = "";
    if (re) err = "Username has already existed";
    let check = true;
    ...
    if (!check) {
        ...
        //if(!dbHandler.checkAlreadyUser(username)) s8 = "Account already existed";
        res.render('register', { err_Empty: { s1, s2, s3, s4, s5, s6, s7,err}})
    }
    else {
